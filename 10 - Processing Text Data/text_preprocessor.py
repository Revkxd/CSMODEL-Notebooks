...
def remove_stopwords(text_series):
    ...
    for text in text_series:
        word_list = [word for word in str(text).split() if word not in stopwords_set]
        text = ' '.join(word_list)
        texts.append(text)
...
def remove_frequent_words(text_series,
    ...
    for text in text_series:
        word_list = [word for word in str(text).split() if word not in frequent_set]
...
def stem(text_series):
    ...
    for text in text_series:
        word_list = [stemmer.stem(word) for word in str(text).split()]
        texts.append(' '.join(word_list))
...
def lemmatize(text_series):
    ...
    for text in text_series:
        ...
        word_list = [lemmatizer.lemmatize(w, map.get(pos[0], wordnet.NOUN)) for w, pos in pos_tagged_text]
        texts.append(' '.join(word_list))
