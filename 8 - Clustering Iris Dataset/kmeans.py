...
class KMeans(object):
    ...
    def group_observations(self, data):
        ...
        # "For each data point" means that we need to get the minimum per row
    ...
    def adjust_centroids(self, data, groups):
        ...
        grouped_data =  grouped_data.groupby('group').mean()
        centroids = grouped_data.iloc[:, self.start_var:self.end_var]
